...
""" @Time    : 2023-08-02
    ...
    Note: These is one wrapper to compute firm-level
    embedding and similarity based on the word2vec model
"""
...
class DocumentSimilarity:
    ...
    def __init__(self, model, gvkeys, conm, keywordslist):
        ...
        '''
        assert len(gvkeys) == len(keywordslist) == len(conm), \
              "gvkeys, conm, keywordslist should should be of the same length"
        ...
        self.document_array = self.document_array / \
            np.linalg.norm(self.document_array, axis=1)[:, np.newaxis]

    def get_firm_embedding(self, firm):
        ''' Given the firm unique identifier, 
            return the embedding of this firm
        '''

        return self.document_embeddings[self.firms.index(firm)]

    def similarity(self, firm1, firm2):
        ''' Given two firms' unique identifiers, 
            return the similarity between the two firms
        '''
    ...
    def most_similar(self, firm, topn = 5):
        '''Given one firm unique identifier, 
        return the topn similar firms to it
        firm: firm unique identifier
        topn: the number of firms to return
        '''
        v = self.document_embeddings[self.firms.index(firm)]    # pylint: disable=invalid-name
        v = v / np.linalg.norm(v)                               # pylint: disable=invalid-name
